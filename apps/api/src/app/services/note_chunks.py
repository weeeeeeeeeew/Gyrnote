...
def parse_openai_embeddings(payload: object, expected_count: int) -> list[list[float]]:
    ...
    for row in rows:
        ...
        if all(item == 0 for item in parsed):
            raise EmbeddingResponseInvalidError("embeddings vector is zero")
...
async def embed_note_texts(texts: list[str]) -> list[list[float]]:
    if not texts:
        return []
    if not settings.EMBEDDING_ENABLED:
        raise EmbeddingNotConfiguredError("note embeddings are disabled")
    api_key = settings.EMBEDDING_API_KEY.get_secret_value() if settings.EMBEDDING_API_KEY is not None else ""
