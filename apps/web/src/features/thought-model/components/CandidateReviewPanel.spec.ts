...
describe('CandidateReviewPanel', () => {
  ...
  it('offers per-node and per-edge accept actions without a bulk overwrite control', () => {
    const wrapper = mount(CandidateReviewPanel, {
      props: { candidate: createCandidate() },
    })

    expect(wrapper.text()).toContain('不会整表覆盖')
    expect(wrapper.text()).not.toMatch(/一键接受全部|整表写入|覆盖确认模型/)
    expect(wrapper.text()).toContain('候选只在此列表')
    expect(wrapper.text()).toContain('接受后会从这里移除')
    expect(wrapper.text()).toContain('确认模型')
    expect(wrapper.find('[aria-label="接受候选节点 candidate-observation-1"]').exists()).toBe(true)
    expect(wrapper.find('[aria-label="接受候选关系 candidate-edge-supports-1"]').exists()).toBe(true)
  })

  it('renders each candidate node with type, text, source anchor and confidence (M3 checkpoint)', () => {
    const wrapper = mount(CandidateReviewPanel, {
      props: {
        candidate: createCandidate({
          nodes: [
    ...
    expect(items[0]?.text()).toContain('anchor-1')
    expect(items[0]?.text()).toContain('0.72')
    expect(items[1]?.text()).toContain('claim')
    expect(items[1]?.text()).toContain('候选主张')
    expect(items[1]?.text()).toContain('anchor-2')
    expect(items[1]?.text()).toContain('0.55')
  })

  it('renders each candidate edge with type, endpoints and confidence', () => {
    const wrapper = mount(CandidateReviewPanel, {
      props: { candidate: createCandidate() },
    })
    ...
    expect(items[0]?.text()).toContain('0.6')
  })
